...
public class Main {
    ...
    public static final String VERSION = "2.6.2-patch";
    ...
    public enum CONFIG_MSG {
		...
		ENEMY_AIM_TRAJECTORIES(106, "&fПредпросмотр траектории снаряда &7→ &fПоказывать траектории врагов"),
        ...
        private CONFIG_MSG(int id, String text) {
			...
			this.displayText = ColorCodesManager.replaceColorCodesInString(text);
			// The protocol id must stay stable for MySerializer, but it should not dictate the
			// order in Forge's config screen. sort_idx follows the grouped enum declaration above.
			this.legacyText = buildConfigKey(id, this.displayText);
			this.text = buildConfigKey(sort_idx, this.displayText);
        }

        public String getName() {
			String s = this.displayText;
        	if (s.charAt(s.length() - 1) == '*') s = s.substring(0, s.length() - 3);
        	return s;
        }
	    };

	private static String buildConfigKey(int stableId, String displayText) {
		return ColorCodesManager.replaceColorCodesInString("&0" + (char) (57344 + stableId) + "&f") + displayText;
	}

	private static final String LEGACY_PEARL_TRAJECTORY_KEY = buildLegacyConfigKey(75, "&fТраектория эндер-перла");
	private static final String LEGACY_ARROW_TRAJECTORY_KEY = buildLegacyConfigKey(76, "&fТраектория стрел");
	...
	private void migrateConfigKeys() {
		ConfigCategory category = config.getCategory(Configuration.CATEGORY_CLIENT);
		for (CONFIG_MSG message : CONFIG_MSG.values()) {
			Property selected = category.get(message.text);
			List<String> oldKeys = new ArrayList<String>();
			for (String key : new ArrayList<String>(category.keySet())) {
				if (key.equals(message.text)) continue;
				if (key.equals(message.legacyText) || key.endsWith(message.displayText)) oldKeys.add(key);
			}
			if (selected == null && !oldKeys.isEmpty()) {
				String oldKey = oldKeys.remove(0);
				selected = category.remove(oldKey);
				selected.setName(message.text);
				category.put(message.text, selected);
			}
			for (String oldKey : oldKeys) category.remove(oldKey);
		}
	}

	private void removeObsoleteConfigProperties() {
		ConfigCategory category = config.getCategory(Configuration.CATEGORY_CLIENT);
		char obsoleteKeyMarker = (char) (57344 + 105);
		for (String key : new ArrayList<String>(category.keySet())) {
			if (key.indexOf(obsoleteKeyMarker) >= 0) category.remove(key);
		}
	}
    ...
    private void initConfig() {
        ...
        config.load();
		migrateConfigKeys();
		removeObsoleteConfigProperties();
        ...
        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.MINIMAP_SIZE.text, 100,                                      ColorCodesManager.replaceColorCodesInString("&fРазмер миникарты &7(от &e50 &7до &e500&7)"));
        ...
        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.TAKE_BLOCKS_FROM_MAX_SLOT.text, false,                        ColorCodesManager.replaceColorCodesInString("&fЕсли у тебя будет в хотбаре больше 1 слота с блоками, будет выбираться слот как можно правее"));
	        ...
	        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.PROJECTILE_TRAJECTORY_PREVIEW.text, previewDefault, ColorCodesManager.replaceColorCodesInString(
	                "&fПоказывает траекторию снаряда в руке: &bэндер-перла&f, стрелы при натяжении лука, снежка, яйца, splash-зелья и бутылочки опыта.\n"
	                        ...
	                        + "&fТакже показывает траектории луков и фаерболов врагов, когда они целятся; &cкрасный &f— траектория попадает в тебя.\n"
	        ...
	        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.ENEMY_AIM_TRAJECTORIES.text, true, ColorCodesManager.replaceColorCodesInString(
	                "&fПоказывает рассчитанную траекторию лука и фаербола в руках врагов.\n"
	                        + "&cКрасная &fлиния проходит рядом с тобой; &6оранжевая &f— летит в другую точку."));
