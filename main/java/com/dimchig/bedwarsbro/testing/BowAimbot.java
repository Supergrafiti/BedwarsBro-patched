			...
			if (hits.size() > 0) {
				...
				if (hits.size() > 0) {
					...
					if (charge > 7000) {
						...
						float target_yaw = (float)(hit.yaw + 360) % 360;
						float target_pitch = (float)hit.pitch;
						float currentYaw = (player_yaw  + 360) % 360;
						float lerpValue = 0.2f;

						float lerp_yaw = currentYaw + lerpValue * (target_yaw - currentYaw);
						float lerp_pitch = player_pitch + lerpValue * (target_pitch - player_pitch);
						//ChatSender.addText(target_yaw - currentYaw);

						HintsFinder.rotateTo(Minecraft.getMinecraft().thePlayer, lerp_yaw, lerp_pitch);
					}
