...
public class HintsBaseRadar {
	...
	public static int basePosX = -9999;
	public static int basePosY = -9999;
	public static int basePosZ = -9999;
	...
	public void updateBooleans() {
		RADAR_RANGE_1 = Main.getConfigInt(CONFIG_MSG.RADAR_RANGE_FIRST);
		RADAR_RANGE_2 = Main.getConfigInt(CONFIG_MSG.RADAR_RANGE_SECOND);
	}
	...
	public static long time_last_message_sent_for_team = 0;
	...
	public static void alertPlayerRange(BWPlayer player, double posY, int range_id, int count, boolean isBedAlert) {
		...
		if (Main.shopManager.findItemInInventory("Наблюдение за") != -1) return;
		...
		if (HintsValidator.isRadarChatMessage() && isBedAlert && new Date().getTime() - time_last_message_sent_for_team > Main.ANTIMUT_DELAY) {
			time_last_message_sent_for_team = new Date().getTime();
			String s = str;
			final String str_final = str;
			
			if (!HintsValidator.isBedwarsMeowColorsActive()) s = ColorCodesManager.removeColorCodes(s);
